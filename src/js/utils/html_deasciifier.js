...
(function() {
  ...
  var inlineTags = {
    // List of inline HTML tags. Anything else will be considered a block
  ...
  function isInlineTag(tag) { return !!inlineTags[tag]; }

  function getTagName(tagValue) {
    // Strip <, > and / from both sides and return the first tag splitted by
    // space:
    function removeFromStart(str, character) {
      return (str.length > 0 && str.charAt(0) == character) ? str.substring(1) :
                                                              str;
    }

    function removeFromEnd(str, character) {
      return (str.length > 0 && str.charAt(str.length - 1) == character) ?
  ...
  var SimpleInnerTextExtractor = {
    getHtmlParseInfo: function(originalHTML) {
      ...
      var lastPos = 0;
      var innerText = "";
      var insertedNewLines =
          [];  // positions of inserted newline characters for block elements
      while ((match = regex.exec(originalHTML)) != null) {
        ...
        if (tagName == "script" || tagName == "noscript" ||
            tagName == "iframe" || tagName == "noframes" ||
            tagName == "frame") {
          throw new Error(
              "Cannot parse html which contains <script>, <noscript>, " +
              "<iframe>, <noframes> or <frame> tags");
        }
      ...
      innerText += originalHTML.substring(
          lastPos);  // text after last tag (or from start if no tags)
    ...
    encodeEntitiesAndRemoveNewLines: function(innerText, decodeInfo,
                                              insertedNewLines) {
      // Put back html entities (e.g.: &nbsp; )
      var reencodedInnerText =
          MEA.HtmlDecoder.restoreDecoded(innerText, decodeInfo);
      ...
      for (var i = 0; i < insertedNewLines.length; i++) {
        ...
        if (splitted[pos] == "\n") {
          ...
          throw new Error("Expected newline character at position " + pos +
                          " of " + reencodedInnerText);
        }
      }
    ...
    rebuildHTML: function(originalHTML, parseInfo, deasciifiedInnerText) {
      ...
      // First, put "&nbsp;" style entities back and remove new lines for block
      // elements
      var reencodedInnerText =
          SimpleInnerTextExtractor.encodeEntitiesAndRemoveNewLines(
              deasciifiedInnerText, parseInfo.innerText.decodeInfo,
              parseInfo.insertedNewLines);
      ...
      var lastTagIndex = 0;
      var chars = [];
      // Compare original html with the deasciified inner text. Replace modified
      // characters in the HTML:
      var outputHTML = originalHTML.split("");
      for (var i = 0; i < reencodedInnerText.length; i++) {
        var htmlPos = offset + i;
        while (lastTagIndex < htmlTags.length &&
               htmlPos >= htmlTags[lastTagIndex].start) {
          // skip tags
  ...
  MEA.HtmlDeasciifier = {
    processHtml: function(html, processFunc) {
      ...
      processResult.text = SimpleInnerTextExtractor.rebuildHTML(
          html, parseInfo, processResult.text);
