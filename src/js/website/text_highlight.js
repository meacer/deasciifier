...
(function(CSS, TEXT_HELPER) {
  ...
  var cssWrapper = [
    "z-index: 5;",
    "position:absolute;",  // This should be absolute to be able to position the
                           // wrapper under the textbox
  ...
  var cssTextArea = [
    ...
    "color:black;",
    "position:relative;"
    //"resize:none;"              // disable resizing, the textbox will auto
    // grow
  ];

  var cssCommon = [
    // Common for textarea and pre
    "position:absolute;",  // TODO: This causes problems with textarea resizing,
                           // so keep it and disable resizing. Autogrow the
                           // textbox
    "left:0;top:0;",
    "right:0;bottom:0;"
  ...
  var cssStyles = [
    ["." + CSS_WRAPPER_DIV, cssWrapper],
    [
      "." + CSS_MIRROR_TEXT,
      [
        cssCommon.join(";") + ";" + cssTextCommon.join(";") + ";" +
        cssMirror.join(";")
      ]
    ],
    [
    ...
    [
      ...
      [
        cssTextCommon.join(";") +
        ";background-color:yellow; display:inline; word-wrap:none;"
      ]
    ]
  ];
  ...
  function sortFunc(a, b) { return a.start - b.start; }
  ...
  /** Represents the areas to be highlighted. Each hiliteArea corresponds to
   * one
   ...
   * actually hilited regions are children of this span. This way the
   * wrapping
  ...
  TEXTHILITE.hiliteArea.prototype = {
    addRange: function(range) { this.hiliteRanges.push(range); },
    ...
    build: function() {
      ...
      for (var i = 0; i < this.hiliteRanges.length; i++) {
        ...
        var rangeStart =
            range.start - this.start;           // relative to the area's start
        var rangeEnd = range.end - this.start;  // relative to the area's start
        var unformattedText =
            escapeHtml(this.text.substring(lastPos, rangeStart));
        ...
        hiliteNode.innerHTML =
            escapeHtml(this.text.substring(rangeStart, rangeEnd));
        childNodes.push(hiliteNode);
        lastPos = rangeEnd;
      }
      if (lastPos < this.text.length) {
        childNodes.push(
            document.createTextNode(escapeHtml(this.text.substring(lastPos))));
      }
  ...
  TEXTHILITE.INSTANCE = function(textArea) {
    ...
    this.wrapperDiv = null;  // The parent <div> element that wraps textarea and
                             // other elements
    this.overlayDiv = null;  // The <div> element that has a high z-index and
                             // shows before others. Used for popup menus.
    this.hiliteLayer = null;  // The element used for highlighting text
    this.cursorLayer =
        null;  // The element used for determining cursor coordinates

    // this.placeholderDiv = null; // This element will be empty. It is used for
    // measuring CSS width/heights.
  ...
  TEXTHILITE.INSTANCE.prototype = {
    getParentNode: function() { return this.wrapperDiv; },

    updateDimensions: function() {

      var textDimensions = CSS.getDimensions(this.textArea);
      if (textDimensions.width == this.prevDimensions.width &&
          textDimensions.height == this.prevDimensions.height) {
        return;
      }
      this.prevDimensions = textDimensions;

      // log("Textarea: Width: " + textDimensions.width + ", Height: "+
      // textDimensions.height);
      // log("Textarea: PrevWidth: " + this.prevDimensions.width + ",
      // PrevHeight: "+ this.prevDimensions.height);
      // log("Textarea: Relative Top: " + textDimensions.relativeTop + ",
      // Relative Left: " + textDimensions.relativeLeft);

      this.wrapperDiv.style.top = textDimensions.relativeTop + "px";
      this.wrapperDiv.style.left = textDimensions.relativeLeft + "px";

      // placeholderDiv has zero width and height but some borders. Based on
      // what
      // we receive from getDimensions, we will use it to adjust the width for
      // different
      // browsers. E.g. if we receive width=0, this means browser ignores
      // borders.
    ...
    createDOM: function(options) {
      ...
      this.textZIndex = 20;  // this.textArea.style.zIndex ?
                             // parseInt(this.textArea.style.zIndex, 10): 10;
      this.options = options;

      // Create the wrapping (parent) div
      this.wrapperDiv = document.createElement("div");
      CSS.setClass(this.wrapperDiv, CSS_WRAPPER_DIV);
      CSS.setStyles(this.wrapperDiv, {
        "zIndex": this.textZIndex
      });  // Send the wrapper div to the background
      ...
      CSS.setStyles(this.hiliteLayer, {
        "zIndex": this.textZIndex - 1
      });  // Send the mirroring pre to the background
      ...
      // Copy css properties starting with the following from textarea to the
      // clone elements:
      var stylesToCopy = [
        "border-left-color",
        "border-left-width",
      ...
      // Make the borders white because on Safari and IE6 they don't match
      // exactly with the textarea:
      var borderStyles = {
    ...
    getText: function() { return this.textArea.value; },

    getHiliteArea: function(startPos) { return this.hiliteAreasMap[startPos]; },
    ...
    /** Rebuilds and displays the highlighted regions for this highlighter's
     * textbox
     */
    update: function() {
      ...
      for (var i = 0; i < this.hiliteRanges.length; i++) {
        ...
        htmlCodes.push("<span class='" + CSS_HILITE_SPAN + "' style='" +
                       hiliteStyles + "'>" + hiliteText + "</span>");
        lastPos = range.end;
      }
      if (lastPos > 0) {
    ...
    getCoordsFromWord: function(wordStart, wordEnd) {
      ...
      var textCursor = fullText.substring(
          wordStart, wordEnd);  // Word between the given coords
      var textAfter = fullText.substring(
          wordEnd);  // Text after cursor, required for word breaks

      var html = escapeHtml(textBefore) + "<span id='" + ID_FOR_CURSOR +
                 "' style='display:inline'>" + escapeHtml(textCursor) +
                 "</span>" + escapeHtml(textAfter);
    ...
    hiliteRange: function(start, end) {
      ...
      // Check if we already have a hiliteArea item for this range. Search for
      // the
      // first whitespace before the start. We assume that there are no
      // whitespaces
      // between start and end:
      var prevWhiteSpace =
          TEXT_HELPER.getPreviousWhiteSpacePos(this.getText(), start);
      ...
      if (existingHiliteArea) {
        ...
        var nextWhiteSpace =
            TEXT_HELPER.getNextWhiteSpacePos(this.getText(), start);
        var hiliteArea =
            this.createHiliteArea(prevWhiteSpace + 1, nextWhiteSpace);
    ...
    /** Clear all highlighted regions if the cursor position is before any of
     * the
     *  regions (such as a new word is entered in the middle of a long text,
     * where
     *  there are highlighted regions after the cursor). We are doing this to
     * prevent
