  ...
  MEA.CSS = {
    ...
    getComputedStyle: function(domNode, styleName, isString) {
      ...
      var view = document.defaultView &&
                 document.defaultView.getComputedStyle(domNode, "");
      if (!view) {
        return value;
      }
    ...
    setWidth: function(domNode, width) { $(domNode)
                                             .width(width); },
    setHeight: function(domNode, height) { $(domNode)
                                               .height(height); },
    getPageOffsetTop: function(domNode) {
      return this.getRelativeOffsetTop(domNode) +
             (domNode.offsetParent ?
                  this.getPageOffsetTop(domNode.offsetParent) :
                  0);
    },
    ...
    getPageOffsetLeft: function(domNode) {
      return this.getRelativeOffsetLeft(domNode) +
             (domNode.offsetParent ?
                  this.getPageOffsetLeft(domNode.offsetParent) :
                  0);
    },

    getRelativeOffsetTop: function(domNode) { return domNode.offsetTop; },

    getRelativeOffsetLeft: function(domNode) { return domNode.offsetLeft; },
    ...
    getStyles: function(domNode) { return this.getComputedStyle(domNode); },
    ...
    _copyStyle: function(destNode, srcNode, styleName) {
      ...
      for (var i=0; i<srcStyles.length; i++) {
        ...
        if (style.indexOf(styleName)==0) {
          var srcValue = this.getComputedStyle(srcNode, style,
      true);//srcNode.style[styleName];
     ...
     * on style names to copy, e.g. ["border*", "text*", "line-height"] will
     * copy
     * all styles starting with 'border' and 'text' and also copy the exact
     * style
    ...
    copyStyles: function(destNode, srcNode, stylesToCopy) {
      ...
      for (var i = 0; i < stylesToCopy.length; i++) {
        ...
        for (var styleName in srcStyles) {
          if (styleName == styleToCopy ||
              (wildcard && styleName.indexOf(styleToCopy) == 0)) {
    ...
    setOuterWidth: function(domNode, width) {
      ...
      this.setStyles(domNode,
                     {"width": (width - (borderLeft + borderRight)) + "px"});
    },
    ...
    setOuterHeight: function(domNode, height) {
      ...
      this.setStyles(domNode,
                     {"height": (height - (borderTop + borderBottom)) + "px"});
    },
