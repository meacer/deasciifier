...
(function(CSS, EVENT_HANDLER) {
  ...
  /** @const */ var CSS_SUGGEST_MENU_BTN_CONTAINER =
      "deasc-suggest-menu-btn-container";
  ...
  /** @const */ var CSS_SUGGEST_ITEM_STATIC_FIRST =
      "deasc-suggest-item-static-first";
  /** @const */ var CSS_SUGGEST_ITEM_STATIC_LAST =
      "deasc-suggest-item-static-last";

  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC = "deasc-suggest-item-dynamic";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM =
      "deasc-suggest-item-dynamic-subitem";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_FIRST =
      "deasc-suggest-item-dynamic-subitem-first";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_LAST =
      "deasc-suggest-item-dynamic-subitem-last";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_BOTTOM =
      "deasc-suggest-item-dynamic-subitem-bottom";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_CHANGED =
      "deasc-suggest-item-dynamic-subitem-changed";
  /** @const */ var CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_SELECTED =
      "deasc-suggest-item-dynamic-subitem-selected";

  function initCSS() {
    CSS.createStyle(
        "." + CSS_SUGGEST_MENU,
        "position:absolute; " + "display:none; " + "line-height: 24px; " +
            "width:auto;" + "background: transparent;" + "border: 0;" +
            "-webkit-user-select: none; -moz-user-select: none; " +
            "padding: 0; margin: 0;" + "word-wrap:none;" + "float:left;" +
            "vertical-align:top;" + "cursor:default;" + "z-index:998;" +
            "box-sizing: border-box;" + "-moz-box-sizing: border-box;" +
            "white-space: nowrap;" + "outline: none; "  // disable focus box
        );
    CSS.createStyle(
        "." + CSS_SUGGEST_MENU_BTN_CONTAINER,
        "display:inline-block; float:left; margin-top:2px; margin-left:3px");
    CSS.createStyle(
        "." + CSS_SUGGEST_MENU_BTN,
        "background: transparent url(static/img/v2.0/button-sprite.png) 0 0;" +
            ...
            "font-family: Tahoma; " + "display:inline-block;" + "border:0;" +
    ...
    CSS.createStyle(
        "." + CSS_SUGGEST_MENU_CONTAINER,
        "position:relative; " + "display:inline-block; " +
            "line-height: 24px; " + "color:#666; cursor: default; " +
            "-webkit-box-shadow: rgb(153, 153, 153) 0px 2px 5px 0px; " +
            "-moz-box-shadow: 0px 2px 5px #999999; " +
            "box-shadow: 0px 2px 5px #999; " + "height: 24px; " +
            "background: transparent;" + "border-top: 1px solid #a1a1a1; " +
            "-webkit-user-select: none; -moz-user-select: none; " +
            "padding: 0; margin: 0;" + "word-wrap:none;" +
            "white-space:nowrap;" + "float:left;" + "vertical-align:top;" +
            "cursor:default;" + "outline: none; "  // disable focus box
        );
    CSS.createStyle("." + CSS_SUGGEST_ITEM,
                    "float:left; " + "background: #ECE9D8; " + "width:16px; " +
                        "text-align:center; " + "text-decoration:none;" +
                        "margin:0; padding:0;" + "vertical-align:top");

    // Static items. These are the characters that do not have alternatives, and
    // don't have submenus:
    CSS.createStyle("." + CSS_SUGGEST_ITEM_STATIC,
                    "border-bottom: 1px solid #AEAEAE; ");
    CSS.createStyle("." + CSS_SUGGEST_ITEM_STATIC_FIRST,
                    "border-left  : 1px solid #aaa");
    CSS.createStyle("." + CSS_SUGGEST_ITEM_STATIC_LAST,
    ...
    CSS.createStyle(
        "." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM + ":hover",
        "background: #3875C9; color:#f8f8f8;");  // Won't work for IE
    CSS.createStyle("." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_FIRST,
                    "border-left:1px solid #aaa");
    CSS.createStyle(
        "." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_LAST,
        "padding-left:1px;border-right:1px solid #aaa");  // padding is for IE
    CSS.createStyle("." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_BOTTOM,
                    "border-left:1px solid #aaa; border-right:1px solid #aaa");
    CSS.createStyle("." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_CHANGED,
                    "background: lightgreen;");
    CSS.createStyle("." + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_SELECTED,
                    "background: lightgreen; ");
  }
  initCSS();
  ...
  MEA.CorrectionMenu = {
    ...
    create: function(parentNode, onSelectHandler, onApplyToAllHandler) {
      var instance =
          new this.INSTANCE(parentNode, onSelectHandler, onApplyToAllHandler);
      instance.init();
      return instance;
    }
  };
   ...
   * @constructor
   */
  MEA.CorrectionMenu.INSTANCE = function(parentNode, onSelectHandler,
                                         onApplyToAllHandler) {
  ...
  MEA.CorrectionMenu.INSTANCE.prototype = {
    ...
    updateSubmenu: function(submenuPos, selectedValue) {
      ...
      this.buildSubmenu(subMenu.domNode, subMenu.alternatives, submenuPos,
                        selectedValue);
    },
    ...
    buildSubmenu: function(parent, items, menuPos, selectedValue) {
      ...
      function addItem(itemValue, itemPos, isTopItem) {
        ...
        CSS.setClass(node,
                     CSS_SUGGEST_ITEM + " " + CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM);
        if (!isTopItem) {
          CSS.addClass(node, CSS_SUGGEST_ITEM_DYNAMIC_SUBITEM_BOTTOM);
        }
        node.innerHTML = itemValue;
        parent.appendChild(node);
        EVENT_HANDLER.bindEvent(
            node, "click",
            getSubmenuItemClickedHandler(menuPos, itemPos, itemValue));
        return node;
      }
    ...
    buildMenu: function(parentNode, horizontalMenuItems) {
      ...
      for (var i = 0; i < horizontalMenuItems.length; i++) {
        ...
        if (item.alternatives) {
          // Build a submenu
          CSS.setClass(child,
                       CSS_SUGGEST_ITEM + " " + CSS_SUGGEST_ITEM_DYNAMIC);
          // Default selected item is the corresponding character from the
          // original text:
    ...
    createButtons: function() {
      ...
      function getResetSelectionHandler() {
        return function() { return instance.resetSelection(); };
      }
      function getApplyAllHandler() {
        return function() { return instance.applySelectionToAll(); };
      }
    ...
    createMenu: function(text) {
      ...
      for (var i = 0; i < text.length; i++) {
        ...
        if (correctionTable[ch]) {  // The character has alternative
                                    // characters
          horizontalMenuItem =
              new MEA.CorrectionMenu.ITEM(ch, correctionTable[ch]);
        } else {
          horizontalMenuItem = new MEA.CorrectionMenu.ITEM(
              ch, null);  // The character has no alternatives
        }
    ...
    isVisible: function() { return (this.domNode.style.display == "block"); }
