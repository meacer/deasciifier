...
(function() {
  ...
  /** @constructor */
  var SelectionRange =
      function(start, end, startRestore, endRestore, isIE) {
    this.start = start;
    this.end = end;
  ...
  // IE Text selection code, Courtesy Tim Down, from
  // http://stackoverflow.com/questions/235411/
  // Modified for IE 9
  function getIEInputSelection(el) {
    var start = 0, end = 0;
    var startOffset,
        endOffset;  // How many characters do start and end skew by?
  ...
  MEA.TextSelection = {
    ...
    getRange: function(textElement) {
      if (document.selection) {
        ...
        // Others
        return createRange(textElement.selectionStart, textElement.selectionEnd,
    ...
    setSelectionText: function(textElement, newText) {
      ...
      textElement.value = textElement.value.substring(0, selection.start) +
                          newText + textElement.value.substring(selection.end);
    ...
    deleteSelectionText: function(textElement) {
      ...
      if (selLength == 0) {
        ...
        textElement.value =
            textElement.value.substring(0, selection.start - 1) +
            textElement.value.substring(selection.end);
        selection.start--;
        selection.end--;
