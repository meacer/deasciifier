...
(function(DEASCIIFIER, ASCIIFIER, EVENT_HANDLER, TEXT_SELECTION, TEXT_HILITE,
          TEXT_HELPER, CORRECTION_MENU, OPTIONS) {
  ...
  var KeyState = {isCTRL: false};
  var MouseState = {isDown: false};
  ...
  var HANDLERS = {
    ...
    onKeyUp: function(textArea, evt) {
      ...
      if (instance) {
        ...
        CTRL+Space keyup popped the corrections menu
          //return false;
  ...
  var uiLang = new MEA.UILang("tr", {
    ...
    "confirmApplyCorrectionToAll": {
      ...
      "tr":
          "Bu de\u011Fi\u015Fikli\u011Fi t\u00FCm metin \u00FCzerinde uygulamak istiyor musunuz?"
    }
  });

  function getUIText(textName) { return uiLang.getText(textName); }

  /** @const */ var OPTION_AUTO_CONVERT = "auto_convert";
  /** @const */ var OPTION_ENABLE_CORRECTIONS =
      "enable_corrections";  // Enables corrections. If not enabled, corrections
                             // are not installed.
  /** @const */ var OPTION_SHOW_CORRECTIONS =
      "show_corrections";  // Displays corrections. Can be turned on/off any
                           // time
  ...
  var defaultOptions = {
    OPTION_AUTO_CONVERT:
        true,  // Turns on/off automatic deasciification while typing
    ...
    OPTION_CONFIRM_FULLTEXT: true,    // If true, asks for confirmation before
                                      // full text deasciification
    OPTION_RESIZEABLE: true           // Can the textbox be resized?
  };

  /** Wrapper object for deasciifier. */
  var DeasciiConverter = {
    process: function(text) { return DEASCIIFIER.deasciify(text); },
  ...
  var AsciiConverter = {
    process: function(text) { return ASCIIFIER.asciify(text); },
    processRange: function(text, start, end) {
      return ASCIIFIER.asciifyRange(text, start, end);
    },
    toString: function() { return "asciify"; }
  };
  ...
  var BOX = {
    ...
    getInstance: function(textArea) { return this.instances[textArea]; },

    setInstance: function(textArea, instance) {
      this.instances[textArea] = instance;
    },

    getInstances: function() { return this.instances; },
    ...
    installEventHandlers: function(textArea) {
      ...
      for (var i = 0; i < handledEvents.length; i++) {
        ...
        EVENT_HANDLER.bindEvent(textArea, evt.name,
                                handler);  // how to remove this?
      }
    },
  ...
  BOX.INSTANCE.prototype = {
    ...
    init: function() {
      ...
      var textHighlightOptions = new OPTIONS({
        ...
        //"hiliteStyles": "background-color:yellow; border-bottom:1px solid
        // blue",
      });
      this.textHilite =
          TEXT_HILITE.install(this.textArea, textHighlightOptions);
      ...
      if (this.options.get(OPTION_ENABLE_CORRECTIONS, true)) {
        ...
        this.correction = {
          ...
          menu: CORRECTION_MENU.create(
              parentNode,
              function(correction) {
                return instance.onSelectCorrection(correction);
              },
              function(correction) {
                return instance.onCorrectionApplyToAll(correction);
              })
        };
      }
    },
    ...
    onCorrectionApplyToAll: function(correction) {
      ...
      if (this.correction && confirm(question)) {
        ...
        var replacedPositions =
            this.replaceAll(correction.originalText, correction.text);
        // Add replaced positions to the changed positions list so that we can
        // display
    ...
    applyCorrection: function(correction) {
      this.setText(TEXT_HELPER.setSubstring(
          this.textArea.value, this.correction.position.start,
          this.correction.position.end, correction.text));
      ...
      if (correction["changedPositions"]) {
        for (var i = 0; i < correction["changedPositions"].length; i++) {
          changedPositions.push(correction["changedPositions"][i] +
                                this.correction.position.start);
        }
      }
      this.highlightChanges(changedPositions, true);  // forceClear:true
    },
    ...
    onMenuKeyDown:function(evt, isCTRL) {
      if (this.correction) {
        ...
        if (false && this.correction.menu.isVisible()) {
          ...
          if (isCTRL && keyCode==EVENT_HANDLER.KeyCodes.SPACE) {   // CTRL +
    Space shows the menu
    ...
    */
    ...
    showCorrectionMenuAtCursor: function() {
      if (!this.textHilite || !this.correction ||
          !this.options.get(OPTION_SHOW_CORRECTIONS, true)) {
        return;
      }
      this.hideCorrectionMenu();
      var selectionRange = TEXT_SELECTION.getRange(this.textArea);
      ...
      if (selectionRange.start == selectionRange.end) {
        ...
        if (TEXT_HELPER.isCursorInsideWord(this.textArea.value, cursorPos) ==
            false) {
          this.hideCorrectionMenu();
          return;
        }
        var wordBoundary =
            TEXT_HELPER.getWordAtCursor(this.textArea.value, cursorPos);
        var wordText =
            this.textArea.value.substring(wordBoundary.start, wordBoundary.end);
        // Don't show menu if there is nothing to suggest
        if (CORRECTION_MENU.hasCorrections(wordText) == false) {
          return;
        }

        var wordCoords = this.textHilite.getCoordsFromWord(wordBoundary.start,
                                                           wordBoundary.end);
    ...
    deasciifyCursor: function() {
      ...
      if (selectionRange.start == selectionRange.end) {
        ...
        rangeToDeasciify = TEXT_HELPER.getWordBeforeCursor(
            this.textArea.value, selectionRange.start);
      } else {
    ...
    processSelection: function(textProcessor) {
      ...
      if (selectionRange.start == selectionRange.end) {
        ...
        result = textProcessor.processRange(
            this.textArea.value, selectionRange.start, selectionRange.end - 1);
      }
      // Highlight the results
      this.displayResults(result);
      // Restore cursor
  ...
  function init() {
    // Add handlers etc
    EVENT_HANDLER.bindEvent(window, "resize",
                            function(e) { BOX.onResizeWindow(e); });
...
})(window["Deasciifier"], window["Asciifier"], MEA.EventHandler,
   MEA.TextSelection, MEA.TextHilite, MEA.TextHelper, MEA.CorrectionMenu,
   MEA.OptionSet);
